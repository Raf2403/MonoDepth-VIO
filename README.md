...
## Изменения в коде OpenVINS

1. **ov_core/src/feat/Feature.h**
   
   - В секцию public добавлена переменная double depth_cnn (глубина от нейросети)

2. **ov_msckf/src/ros/ROS2Visualizer.h**
   
   - В секцию public добавлено объявление коллбэк-функции void callback_depth   
   - В секцию protected добавлена переменная для подписчика sub_depth

3. **ov_msckf/src/ros/ROS2Visualizer.cpp**
   
   - В функцию setup_subscribers добавлена подписка на топик /depth/image_raw
   - Реализовано тело функции callback_depth
  
    Теперь ROS2Visualizer принимает топик, конвертирует в OpenCV и вызывает feed_measurement_depth.

4. **ov_msckf/src/core/VioManager.h**
   
   - В секцию public добавлено объявление метода feed_measurement_depth
   - В секцию protected добавлена структура для хранения последних карт глубины depth_buffer и мьютекс (механизм синхронизации, который гарантирует, что в каждый момент времени только один поток имеет доступ к определенному ресурсу) depth_buffer_mtx для защиты этого буффера 

5. **ov_msckf/src/core/VioManager.cpp**
   
   - Реализовано тело функции feed_measurement_depth
   - В функции track_image_and_update добавлена глубина с нейросети к фичам
    
    Теперь VioManager сохраняет картинку в буфер. При обработке кадра камеры (track_image_and_update) мы находим эту картинку, берем координаты всех точек трекера, смотрим их глубину и пишем в поле depth_cnn класса Feature. Теперь, когда фича попадет в UpdaterMSCKF, у неё внутри уже будет лежать глубина depth_cnn.

6. **open_vins/ov_msckf/src/update/UpdaterMSCKF.cpp**
   
   Этот файл отвечает за математику фильтра Калмана в OpenVINS. Нужно, чтобы значение глубины с нейросети (depth_cnn) влияло на оценку траектории.

    Раньше алгоритм использовал фиксированный шум для всех ключевых точек. 
    
    Теперь же была внедрена адаптивная ковариацая шума измерения, которая сравнивает два источника глубины: нейросетевая и глубина, рассчитанная методом триангуляции на основе движения камеры и позиций фич. 
    - Если относительная разница между этими двумя глубинами меньше 20%, то алгоритм уменьшает шум измерения в 2 раза (нейросеть подтверждает расчеты VIO). 
    - Если эта разница больше 50%, то алгоритм увеличивает шум измерения в 100 раз (возможно, это динамический объект, сложное отражение или ошибка триангуляции).
    
    Это позволяет фильтру использовать надежные статичные структуры (стены, пол), которые подтверждает нейросеть, и игнорировать сложные или движущиеся объекты.

## **Сборка проекта после изменений**

   Замените файлы по указанным директориям, после чего пересоберите изменные пакеты проекта
   ```bash
   source /opt/ros/humble/setup.bash
   cd ~/ros2_ws
