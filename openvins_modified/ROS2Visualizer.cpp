...
void ROS2Visualizer::setup_subscribers(std::shared_ptr<ov_core::YamlParser> parser) {
  ...
  std::string topic_depth = "/depth/image_raw"; // Можно вынести в параметры, но пока жестко зададим
  // _node->declare_parameter<std::string>("topic_depth", "/depth/image_raw");
  // _node->get_parameter("topic_depth", topic_depth);
  
  sub_depth = _node->create_subscription<sensor_msgs::msg::Image>(
      topic_depth, rclcpp::SensorDataQoS(),
      std::bind(&ROS2Visualizer::callback_depth, this, std::placeholders::_1));
  PRINT_INFO("subscribing to Depth: %s\n", topic_depth.c_str());
...
void ROS2Visualizer::callback_depth(const sensor_msgs::msg::Image::SharedPtr msg) {
  
  // 1. Convert timestamp
  double timestamp = msg->header.stamp.sec + msg->header.stamp.nanosec * 1e-9;

  // 2. Convert ROS -> OpenCV
  cv_bridge::CvImagePtr cv_ptr;
  try {
    // Мы ожидаем 32FC1 (float метры), как отправляет depth_publisher
  ...
  // 3. Debug print (чтобы убедиться, что работает)
  PRINT_INFO("Received depth image at %.4f\n", timestamp);

  // 4. Send to VioManager (TODO: Эту функцию мы создадим на следующем шаге!)
  _app->feed_measurement_depth(timestamp, cv_ptr->image);
}
