...
void VioManager::track_image_and_update(const ov_core::CameraData &message_const) {
  ...
  trackFEATS->feed_new_camera(message);
    // =====================================================================
    // ATTACH DEPTH TO FEATURES (DEBUG VERSION)
    // =====================================================================
    {
        std::lock_guard<std::mutex> lck(depth_buffer_mtx);
        
        // Отладка: Состояние буфера
        PRINT_INFO("VioManager: Track time %.4f. Depth Buffer size: %d\n", message.timestamp, (int)depth_buffer.size());
        
        // 1. Ищем карту глубины с увеличенным допуском (0.05 сек = 50 мс)
        double best_dt = 0.05; 
        cv::Mat best_depth_img;
        bool found_depth = false;

        // Ищем ближайший элемент (lower_bound ищет >=, поэтому проверяем его и предыдущий)
        auto it = depth_buffer.lower_bound(message.timestamp);
        
        // Проверяем элемент, который нашли (>= timestamp)
        if (it != depth_buffer.end() && std::abs(it->first - message.timestamp) < best_dt) {
             best_depth_img = it->second;
             found_depth = true;
        }
        // Проверяем предыдущий элемент (< timestamp), вдруг он ближе
        else if (it != depth_buffer.begin()) {
            auto prev_it = std::prev(it);
            if (std::abs(prev_it->first - message.timestamp) < best_dt) {
                best_depth_img = prev_it->second;
                found_depth = true;
            }
        }

        if (found_depth) {
            PRINT_INFO("VioManager: MATCH FOUND! Depth image attached.\n");
        } else {
            // Если буфер не пуст, напечатаем, насколько мы промахнулись
            if (!depth_buffer.empty()) {
                double last_time = depth_buffer.rbegin()->first;
                double diff = message.timestamp - last_time;
                PRINT_INFO("VioManager: NO MATCH. Last depth was %.4f ago. (lag)\n", diff);
            }
        }

        // 2. Если карта глубины есть, привязываем к фичам
        if (found_depth && !best_depth_img.empty()) {
            
            auto feature_db = trackFEATS->get_feature_database();
            std::vector<std::shared_ptr<ov_core::Feature>> active_feats = feature_db->features_containing(message.timestamp);
            
            int attached_count = 0;

            for (auto &feat : active_feats) {
                // Работаем с камерой 0 (левая / моно)
                size_t cam_id = 0;
                
                // Проверяем, видела ли эта фича камеру 0
                auto it_times = feat->timestamps.find(cam_id);
                if (it_times != feat->timestamps.end()) {
                    
                    const auto& times = it_times->second;
                    const auto& measurements = feat->uvs.at(cam_id);
                    
                    // Ищем индекс, соответствующий текущему времени
                    for (int i = times.size() - 1; i >= 0; i--) {
                        if (std::abs(times[i] - message.timestamp) < 1e-5) {
                            
                            // Нашли измерение!
                            // Кастим к double, так как VectorXf
                            Eigen::Vector2d uv = measurements[i].head<2>().template cast<double>();
                            
                            int u = std::round(uv(0));
                            int v = std::round(uv(1));

                            // Проверка границ изображения
                            if (u >= 0 && v >= 0 && u < best_depth_img.cols && v < best_depth_img.rows) {
                                float d = best_depth_img.at<float>(v, u);
                                
                                // Проверка валидности глубины
                                if (d > 0.1 && std::isfinite(d)) {
                                    feat->depth_cnn = (double)d;
                                    attached_count++;
                                }
                            }
                            break; // Измерение найдено
                        }
                    }
                }
            }
            ...
            if (attached_count > 0) {
                PRINT_INFO("VioManager: Attached CNN depth to %d features.\n", attached_count);
            }
        }
    }
...
void VioManager::feed_measurement_depth(double timestamp, const cv::Mat &depth_image) {
    std::lock_guard<std::mutex> lck(depth_buffer_mtx);
    // Сохраняем копию картинки
    depth_buffer[timestamp] = depth_image.clone();

    // Очистка старых данных (храним последние 2 секунды)
    auto it = depth_buffer.begin();
    while (it != depth_buffer.end()) {
        if (it->first < timestamp - 2.0) {
            it = depth_buffer.erase(it);
        } else {
            ++it;
        }
    }
